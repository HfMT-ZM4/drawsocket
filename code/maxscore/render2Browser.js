    ...
    switch (messagename) {
        ...
        case "StaffLine":
			//StaffLine measureIndex staffIndex staffLineIndex zoom x1 y1 x2 y2 selected
 			stafflines[msg[0]][msg[1]][msg[2]] = [msg[4], msg[5], msg[6], msg[7]];
 ...
 //           var element = keys[k].substr(0, keys[k].indexOf('_'));
       ...
       case "endRenderDump":
		...
		var multiple = glyph.length / 5;
            ...
            svgGroups[s + 1]["/" + c + "/style/font-family"] = glyph[i*5+3];
            svgGroups[s + 1]["/" + c + "/style/font-size"] = glyph[i*5+4];
			if (typeof glyph[i*5+0] == "number") svgGroups[s + 1]["/" + c + "/draw/text"] = [glyph[i*5+1] + msg[0], glyph[i*5+2] + dest[d], glyph[i*5+0].toString()];
            else {
				if (glyph[0].length == 1) svgGroups[s + 1]["/" + c + "/draw/music"] = [glyph[i*5+1] + msg[0], glyph[i*5+2] + dest[d], glyph[i*5+0].charCodeAt(0).toString(16)];
				else svgGroups[s + 1]["/" + c + "/draw/text"] = [glyph[i*5+1] + msg[0], glyph[i*5+2] + dest[d], glyph[i*5+0]];
 				}
           	svgGroups[s + 1]["/" + c + "/transform/matrix"] = [1., 0., 0., 1., 0., 0.];
			}
			}
