...
var drawing = d3.select("#main-drawing"); // svg object
...
function _getBBox(_node)
{
  // In Firefox, getBBox fails if the node is hidden, or not attached
  var oldStyle = _node.style;
  _node.style.display = "block";
  _node.style.visible = false;

  back.append( ()=>{ return _node; } );

  var bb = _node.getBBox();

  console.log("bb", bb);

  _node.style = oldStyle;
...
//        console.log("draw/music", argc);
    ...
    { // return node to add to objectStack
      ...
      {
        const importedElement = d3.select( fileObj[ objValue[0] ] ).select( "#"+ objValue[1] );
        var _importedNode = importedElement.node().cloneNode(1);
//            d3.select(el).append(function(){return svg.node();});
        _importedNode.id += '~';

        if( typeof defStack[ objValue[1] ] != "undefined" )
          defStack[ objValue[1] ].remove();

        defStack[ objValue[1] ] = maindef.append( () =>{
//          console.log("return append");
          return _importedNode; //svgObj(cmdtype, argc, objValue);
        });


        var newNode = d3.create("svg:use").attr( "xlink:href", "#"+_importedNode.id );

        try {
          const bbox = _importedNode.getBBox(); //
          defStack[ objValue[1] ].attr('style', `transform:translate(-${bbox.x}px,-${bbox.y}px); -webkit-transform:translate(-${bbox.x}px,-${bbox.y}px)`);
        } catch (e) {
          console.log( "known issue: Firefox fails on getBBox() to offset imported object.");
        }
...
//  in general: use inline style for any CSS accessable attribute
...
var mainSVG = document.getElementById("main-drawing");
  ...
  for(var node of _obj)
  {
    ...
    for( var prop in node)
    {
      if( prop !== 'svg' )
      {
        if(prop === "style" )
        {
          var cssnode = node[prop];
          for( var cssprop in cssnode )
          {
...
//        console.log(drawingTarget);
      ...
      case "tween/timeline":
        ...
        if( argc === "obj" ) // bundle containing /tweens bundle array, and an optional /params bundle for init
        {
          ...
          for( var tween of objValue.tweens )
          {
